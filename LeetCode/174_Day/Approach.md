...
>   $$\text{new\_flat\_index} = (\text{flat\_index} + k) \pmod{m \times n}$$
> - The new flat index maps back to 2D coordinates:
>   $$\text{new\_row} = \text{new\_flat\_index} \mathbin{/} n$$
>   $$\text{new\_col} = \text{new\_flat\_index} \pmod n$$
> 
...
1. **Initialize Parameters:**
   ...
   - Reduce $k$ using modulo arithmetic: $k = k \pmod{\text{total}}$. If $k = 0$, return the grid immediately as no shifting changes the grid.
...
2. **Compute New Grid Positions:**
   ...
   - Iterate through every cell $(i, j)$ in the original grid:
     ...
     - Find the new 1D index: $\text{new\_flat\_idx} = (\text{flat\_idx} + k) \pmod{\text{total}}$.
     - Store the value at the computed 2D cell:
       $$\text{result}[\text{new\_flat\_idx} \mathbin{/} n][\text{new\_flat\_idx} \pmod n] = \text{grid}[i][j]$$
...
| Element | 2D Coords $(i, j)$ | Flat Index $(i \times n + j)$ | New Flat Index $(+1) \pmod 9$ | New 2D Coords $(\text{div } n, \text{mod } n)$ |
| :---: | :---: | :---: | :---: | :---: |
| 1 | $(0, 0)$ | 0 | 1 | $(0, 1)$ |
| 2 | $(0, 1)$ | 1 | 2 | $(0, 2)$ |
| 3 | $(0, 2)$ | 2 | 3 | $(1, 0)$ |
