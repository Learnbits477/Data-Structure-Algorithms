...
| :------------------- | :--------------- | :---------------------------------------------------------------------------- |
| **Time Complexity**  | $\mathcal{O}((N + M) \log_{10}(\text{max\_val}))$ | For each element in both arrays, we perform at most $\log_{10}(\text{max\_val})$ divisions and set operations. With $\text{max\_val} \le 10^8$, this is at most $9$ iterations per number. |
| **Space Complexity** | $\mathcal{O}(N \log_{10}(\text{max\_val}))$ | The hash set stores up to $9 \times N$ unique prefixes from the first array. |
