...
- For each piece level $$p$$, compute the prefix/suffix sum helper matrices:
...
flowchart TD
    ...
    LoopP -- No --> ReturnResult["Return dp[0][0]"]
...
- Let's compute `dp[0][0]`:
  - `i_first = next_row_with_1[0][0] + 1 = 1 + 1 = 2` (Out of bounds, no horizontal cut).
  - `j_first = next_col_with_1[0][0] + 1 = 0 + 1 = 1` (Valid vertical cut after column 0).
...
| 💾 Space | $$O(n \cdot m)$$ | We only need the DP table of the previous iteration to compute the current one, enabling us to use a rolling buffer of size $$2 \times n \times m$$. |
