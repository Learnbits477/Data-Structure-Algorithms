...
class Solution {
  public:
    void dfs(int u, int parent, vector<int> adj[], vector<int>& tin, vector<int>& low,
        ...
        for (int v : adj[u]) {
            ...
            if (!vis[v]) {
                ...
                // Articulation Point Condition for internal nodes
    ...
    vector<int> articulationPoints(int V, vector<vector<int>>& edges) {
        // Preprocessing: Convert edge list to adjacency list
        vector<int> adj[V];
        for (auto& edge : edges) {
        ...
        sort(res.begin(), res.end()); // Requirement for articulation points
        return res;
    }
};
