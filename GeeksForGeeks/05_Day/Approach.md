...
2. Iterate through the array `arr` from `i = 0` to `n-1`:
   - **Remove outgoing elements:** If the element at the front of the deque is out of the current window range (i.e., `deque.front() <= i - k`), remove it from the front.
...
```mermaid
flowchart TD
    Start([Start]) --> Init["Initialize Deque dq & Result array res"]
    Init --> Loop{"For i from 0 to N-1"}
    Loop -- Yes --> CheckWindow{"dq not empty &<br/>dq.front() == i - k?"}
    CheckWindow -- Yes --> PopFront["dq.pop_front()"] --> CheckSmaller
    CheckWindow -- No --> CheckSmaller{"dq not empty &<br/>arr[dq.back()] <= arr[i]?"}
    
    CheckSmaller -- Yes --> PopBack["dq.pop_back()"] --> CheckSmaller
    CheckSmaller -- No --> PushBack["dq.push_back(i)"]
    
    PushBack --> CheckAns{"i >= k - 1?"}
    CheckAns -- Yes --> AddRes["Add arr[dq.front()] to res"] --> Loop
    CheckAns -- No --> Loop
    
    Loop -- No --> ReturnResult(["Return res"])
```

## ⏳ Complexity Analysis

- **Time Complexity:** $\mathcal{O}(N)$. Each element is added to the deque once and removed at most once.
- **Space Complexity:** $\mathcal{O}(k)$. The deque stores at most `k` indices representing the current window.
